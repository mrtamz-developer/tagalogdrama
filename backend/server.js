...
function rateLimit(req, res, next) { const now=Date.now(); const key=req.ip||req.socket.remoteAddress||'unknown'; const bucket=rateBuckets.get(key); if(!bucket||now-bucket.startedAt>=RATE_WINDOW_MS){rateBuckets.set(key,{startedAt:now,count:1});return next();} bucket.count+=1; if(bucket.count>RATE_LIMIT)return res.status(429).json({error:'Too many requests'}); return next(); }
app.use(rateLimit);
app.get('/health', (_req,res)=>res.json({ok:true,service:'tagalogdrama-api'}));
app.post('/payments/webhook', express.raw({type:'application/json',limit:'100kb'}), async(req,res)=>{const signature=req.headers['paymongo-signature'];const secret=process.env.PAYMONGO_WEBHOOK_SECRET;if(!secret)return res.status(503).json({error:'Payment webhook is not configured'});let payload;try{const rawBody=req.body.toString('utf8');payload=JSON.parse(rawBody);const livemode=payload?.data?.attributes?.livemode===true;if(!verifyWebhookSignature(rawBody,signature,secret,livemode))return res.status(401).json({error:'Invalid webhook signature'});}catch{return res.status(400).json({error:'Invalid webhook payload'});}if(payload?.data?.attributes?.type!=='checkout_session.payment.paid')return res.status(200).json({received:true,ignored:true});try{const result=await fulfillPayMongoCheckoutPaid(payload);return res.status(200).json({received:true,...result});}catch(error){console.error('PayMongo fulfillment failed',error.message);return res.status(503).json({error:'Payment fulfillment temporarily unavailable'});}});
app.get('/ads/admob/ssv',async(req,res)=>{try{await fulfillAdMobReward(`${req.protocol}://${req.get('host')}${req.originalUrl}`);return res.status(200).send('OK');}catch(error){console.error('AdMob SSV verification failed',error.message);return res.status(400).send('Invalid reward');}});
...
app.get('/episodes/:id/access',auth,async(req,res)=>{try{return res.json(await getEpisodeAccess(req.user.id,req.params.id));}catch(error){console.error('Episode access check failed',error.message);return res.status(503).json({error:'Access check temporarily unavailable'});}});
app.post('/episodes/:id/play',auth,async(req,res)=>{const data=await catalog();const episode=data.series.flatMap(s=>s.episodes).find(e=>e.id===req.params.id);if(!episode)return res.status(404).json({error:'Episode not found'});if(!episode.videoUrl)return res.status(503).json({error:'Video not configured yet'});try{const access=await getEpisodeAccess(req.user.id,episode.id);if(!access.allowed)return res.status(403).json({error:'Episode access denied',reason:access.reason});if(!DEMO_MODE)return res.status(501).json({error:'Protected playback is not configured'});return res.json({episodeId:episode.id,playbackUrl:episode.videoUrl,expiresIn:300,access});}catch(error){console.error('Playback access check failed',error.message);return res.status(503).json({error:'Playback authorization temporarily unavailable'});}});
...
app.post('/subscriptions/checkout',auth,async(req,res)=>{const parsed=planSchema.safeParse(req.body);if(!parsed.success)return res.status(400).json({error:'Invalid plan'});if(!process.env.PAYMONGO_SECRET_KEY)return res.status(503).json({error:'Payment provider not configured'});const frontend=process.env.FRONTEND_ORIGIN;if(!frontend)return res.status(503).json({error:'Frontend origin is not configured'});try{const result=await createCheckoutSession({plan:parsed.data.plan,userId:req.user.id,email:req.user.email,successUrl:`${frontend}/payment-success.html`,cancelUrl:`${frontend}/subscription.html`,idempotencyKey:`td-${req.user.id}-${parsed.data.plan}-${String(req.headers['idempotency-key']||Date.now())}`});return res.status(201).json(result);}catch(error){console.error('PayMongo checkout creation failed',error.message);return res.status(502).json({error:'Unable to create payment checkout'});}});
